...
## 1.2 DEMA - Double Exponential Moving Average  双移动平均线
...
## 1.4 HT_TRENDLINE - Hilbert Transform - Instantaneous Trendline
...
## 1.5 KAMA - Kaufman Adaptive Moving Average 考夫曼的自适应移动平均线
...
## 1.7 MAMA - MESA Adaptive Moving Average
...
## 1.12 SAREXT - Parabolic SAR - Extended
```python
real = SAREXT(high, low, startvalue=0, offsetonreverse=0, accelerationinitlong=0, accelerationlong=0, accelerationmaxlong=0, accelerationinitshort=0, accelerationshort=0, accelerationmaxshort=0)
```

## 1.13 SMA - Simple Moving Average 简单移动平均线
...
## 1.14 T3 - Triple Exponential Moving Average (T3) 三重指数移动平均线
...
## 1.15 TEMA - Triple Exponential Moving Average
...
# 2 Momentum Indicator Functions 动量指标函数 
## 2.1 ADX - Average Directional Movement Index
> 函数名：ADX  
名称：平均趋向指数  
...
## 2.2  ADXR- Average Directional Movement Index Rating
> 函数名：ADXR  
名称：平均趋向指数的趋向指数  
...
## 2.3 APO - Absolute Price Oscillator
```python
...
## 2.8 CMO - Chande Momentum Oscillator 钱德动量摆动指标
...
## 2.9 DX - Directional Movement Index DMI指标又叫动向指标或趋向指标
...
## 2.11 MACDEXT - MACD with controllable MA type
...
## 2.12 MACDFIX - Moving Average Convergence/Divergence Fix 12/26
...
## 2.13 MFI - Money Flow Index 资金流量指标
...
## 2.14 MINUS_DI - Minus Directional Indicator
...
## 2.15 MINUS_DM - Minus Directional Movement
...
## 2.17 PLUS_DI - Plus Directional Indicator
...
## 2.18 PLUS_DM - Plus Directional Movement
...
## 2.19 PPO - Percentage Price Oscillator 价格震荡百分比指数
...
## 2.20 ROC - Rate of change : ((price/prevPrice)-1)*100  变动率指标
...
## 2.21 ROCP - Rate of change Percentage: (price-prevPrice)/prevPrice
```python
...
## 2.24 RSI - Relative Strength Index  相对强弱指数
...
## 2.25 STOCH - Stochastic 随机指标,俗称KD 
...
## 2.28 TRIX - 1-day Rate-Of-Change (ROC) of a Triple Smooth EMA
```python
...
## 2.29 ULTOSC - Ultimate Oscillator 终极波动指标 
...
# 3 Volatility Indicator Functions 波动率指标函数  
...
## 3.2 NATR - Normalized Average True Range
> 函数名：NATR  
名称：归一化波动幅度均值   
简介：归一化波动幅度均值（NATR)是
...
# 4 Volume Indicators 成交量指标

## 4.1 AD - Chaikin A/D Line 量价指标
> 函数名：AD  
名称：Chaikin A/D Line 累积/派发线（Accumulation/Distribution Line）  
简介：Marc Chaikin提出的一种平衡交易量指标，以当日的收盘价位来估算成交流量，用于估定一段时间内该证券累积的资金流量。  
计算公式：   
...
# 5 Price Transform Functions 价格指标

## 5.1 AVGPRICE - Average Price
> 函数名：AVGPRICE   
名称：平均价格函数
```python
...
## 5.4 WCLPRICE - Weighted Close Price
> 函数名：WCLPRICE   
名称：加权收盘价
...
# 6 Cycle Indicator Functionsc 周期指标

## 6.1 HT_DCPERIOD - Hilbert Transform - Dominant Cycle Period
> 函数名：HT_DCPERIOD   
名称： 希尔伯特变换-主导周期   
...
## 6.2 HT_DCPHASE - Hilbert Transform - Dominant Cycle Phase
...
## 6.3 HT_PHASOR - Hilbert Transform - Phasor Components
> 函数名：HT_DCPHASE   
...
## 6.4 HT_SINE - Hilbert Transform - SineWave
...
## 6.5 HT_TRENDMODE - Hilbert Transform - Trend vs Cycle Mode
...
# 7 Pattern Recognition Functions 形态识别指标
   
## 7.1 CDL2CROWS - Two Crows
> 函数名：CDL2CROWS   
名称：Two Crows 两只乌鸦   
...
## 7.2 CDL3BLACKCROWS - Three Black Crows
> 函数名：CDL3BLACKCROWS   
...
## 7.3 CDL3INSIDE - Three Inside Up/Down
> 函数名：CDL3INSIDE   
...
## 7.4 CDL3LINESTRIKE - Three-Line Strike 
> 函数名：CDL3LINESTRIKE   
...
## 7.5 CDL3OUTSIDE - Three Outside Up/Down
> 函数名：CDL3OUTSIDE  
...
## 7.6 CDL3STARSINSOUTH - Three Stars In The South
...
## 7.7 CDL3WHITESOLDIERS - Three Advancing White Soldiers
...
## 7.8 CDLABANDONEDBABY - Abandoned Baby
...
## 7.9 CDLADVANCEBLOCK - Advance Block
> 函数名：CDLADVANCEBLOCK   
...
## 7.12 CDLCLOSINGMARUBOZU - Closing Marubozu
...
## 7.13 CDLCONCEALBABYSWALL - Concealing Baby Swallow
...
## 7.14 CDLCOUNTERATTACK - Counterattack
...
## 7.15 CDLDARKCLOUDCOVER - Dark Cloud Cover
...
## 7.18 CDLDRAGONFLYDOJI - Dragonfly Doji
...
## 7.19 CDLENGULFING - Engulfing Pattern
> 函数名：CDLENGULFING  
...
## 7.20 CDLEVENINGDOJISTAR - Evening Doji Star
...
## 7.21 CDLEVENINGSTAR - Evening Star
> 函数名：CDLEVENINGSTAR  
名称：Evening Star 暮星  
...
## 7.22 CDLGAPSIDESIDEWHITE - Up/Down-gap side-by-side white lines
...
## 7.23 CDLGRAVESTONEDOJI - Gravestone Doji
...
## 7.25 CDLHANGINGMAN - Hanging Man
> 函数名：CDLHANGINGMAN  
名称：Hanging Man 上吊线  
...
## 7.27 CDLHARAMICROSS - Harami Cross Pattern
> 函数名：CDLHARAMICROSS  
...
## 7.28 CDLHIGHWAVE - High-Wave Candle
> 函数名：CDLHIGHWAVE  
...
## 7.29 CDLHIKKAKE - Hikkake Pattern
> 函数名：CDLHIKKAKE  
...
## 7.30 CDLHIKKAKEMOD - Modified Hikkake Pattern
> 函数名：CDLHIKKAKEMOD  
...
## 7.31 CDLHOMINGPIGEON - Homing Pigeon
> 函数名：CDLHOMINGPIGEON  
名称：Homing Pigeon 家鸽  
...
## 7.32 CDLIDENTICAL3CROWS - Identical Three Crows
...
## 7.33 CDLINNECK - In-Neck Pattern
>函数名：CDLINNECK  
...
## 7.34 CDLINVERTEDHAMMER - Inverted Hammer
...
## 7.36 CDLKICKINGBYLENGTH - Kicking - bull/bear determined by the longer marubozu
...
## 7.37 CDLLADDERBOTTOM - Ladder Bottom 
> 函数名：CDLLADDERBOTTOM  
名称：Ladder Bottom 梯底  
...
## 7.38 CDLLONGLEGGEDDOJI - Long Legged Doji
...
## 7.39 CDLLONGLINE - Long Line Candle
> 函数名：CDLLONGLINE  
...
## 7.41 CDLMATCHINGLOW - Matching Low
> 函数名：CDLMATCHINGLOW  
...
## 7.43 CDLMORNINGDOJISTAR - Morning Doji Star
...
## 7.44 CDLMORNINGSTAR - Morning Star
> 函数名：CDLMORNINGSTAR  
名称：Morning Star 晨星  
...
## 7.45 CDLONNECK - On-Neck Pattern
> 函数名：CDLONNECK  
...
## 7.46 CDLPIERCING - Piercing Pattern
> 函数名：CDLPIERCING  
...
## 7.47 CDLRICKSHAWMAN - Rickshaw Man
> 函数名：CDLRICKSHAWMAN  
...
## 7.48 CDLRISEFALL3METHODS - Rising/Falling Three Methods
...
## 7.49 CDLSEPARATINGLINES - Separating Lines
...
## 7.50 CDLSHOOTINGSTAR - Shooting Star
> 函数名：CDLSHOOTINGSTAR  
...
## 7.51 CDLSHORTLINE - Short Line Candle
> 函数名：CDLSHORTLINE  
...
## 7.52 CDLSPINNINGTOP - Spinning Top
> 函数名：CDLSPINNINGTOP  
名称：Spinning Top 纺锤  
简介：一日K线，实体小。  
...
## 7.53 CDLSTALLEDPATTERN - Stalled Pattern
...
## 7.54 CDLSTICKSANDWICH - Stick Sandwich
...
## 7.55 CDLTAKURI - Takuri (Dragonfly Doji with very long lower shadow)
> 函数名：CDLTAKURI  
...
## 7.57 CDLTHRUSTING - Thrusting Pattern
> 函数名：CDLTHRUSTING  
...
## 7.58 CDLTRISTAR - Tristar Pattern
> 函数名：CDLTRISTAR  
...
## 7.59 CDLUNIQUE3RIVER - Unique 3 River
> 函数名：CDLUNIQUE3RIVER  
...
## 7.60 CDLUPSIDEGAP2CROWS - Upside Gap Two Crows
...
## 7.61 CDLXSIDEGAP3METHODS - Upside/Downside Gap Three Methods
...
## 8.2 CORREL - Pearson's Correlation Coefficient (r)
> 函数名：CORREL  
名称：皮尔逊相关系数   
...
## 8.3 LINEARREG - Linear Regression
...
## 8.4 LINEARREG_ANGLE - Linear Regression Angle
> 函数名：LINEARREG_ANGLE  
名称：线性回归的角度   
简介：来确定价格的角度变化. 
...
## 8.5 LINEARREG_INTERCEPT - Linear Regression Intercept
...
## 8.6 LINEARREG_SLOPE - Linear Regression Slope
> 函数名：LINEARREG_SLOPE  
名称：线性回归斜率指标
...
# 9 Math Functions 数学运算
## 9.1 ACOS - Vector Trigonometric ACos
> 函数名：ACOS 
...
## 9.2 ASIN - Vector Trigonometric ASin
> 函数名：ASIN 
名称：反正弦函数，三角函数
...
## 9.3 ATAN - Vector Trigonometric ATan
> 函数名：ASIN 
名称：数字的反正切值，三角函数
...
## 9.5 COS - Vector Trigonometric Cos
> 函数名：COS 
名称：余弦函数，三角函数
```python
real = COS(close)
...
## 9.6 COSH - Vector Trigonometric Cosh
> 函数名：COSH 
名称：双曲正弦函数，三角函数
```python
real = COSH(close)
...
## 9.11 SIN - Vector Trigonometric Sin
> 函数名：SIN 
名称：正弦函数，三角函数
```python
real = SIN(close)
...
## 9.12 SINH - Vector Trigonometric Sinh
> 函数名：SINH 
名称：双曲正弦函数，三角函数
...
## 9.14 TAN - Vector Trigonometric Tan
> 函数名：TAN 
名称：正切函数，三角函数
```python
real = TAN(close)
...
## 9.15 TANH - Vector Trigonometric Tanh
> 函数名：TANH 
名称：双曲正切函数，三角函数
```python
real = TANH(close)
...
## 9.16 ADD - Vector Arithmetic Add
> 函数名：ADD   
名称：向量加法运算
...
## 9.17 DIV - Vector Arithmetic Div
> 函数名：DIV   
名称：向量除法运算
```python
real = DIV(high, low)
...
## 9.18 MAX - Highest value over a specified period
> 函数名：MAX   
...
## 9.19 MAXINDEX - Index of highest value over a specified period
> 函数名：MAXINDEX   
名称：周期内最大值的索引   
```python
...
## 9.20 MIN - Lowest value over a specified period
> 函数名：MIN   
...
## 9.21 MININDEX - Index of lowest value over a specified period
> 函数名：MININDEX   
名称：周期内最小值的索引 
```python
...
## 9.22 MINMAX - Lowest and highest values over a specified period
> 函数名：MINMAX   
...
## 9.23 MINMAXINDEX - Indexes of lowest and highest values over a specified period
> 函数名：MINMAX   
...
## 9.24 MULT - Vector Arithmetic Mult
> 函数名：MULT   
名称：向量乘法运算
```python
real = MULT(high, low)
...
## 9.25 SUB - Vector Arithmetic Substraction
> 函数名：SUB   
名称：向量减法运算
```python
real = SUB(high, low)
